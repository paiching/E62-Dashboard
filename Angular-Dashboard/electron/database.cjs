...
function seedDatabase() {
  ...
  const menus = [
    ['overview','儀錶板','◎','overview',10,'dashboard.view'],['trends','趨勢圖','⌁','trends',20,'trends.view'],
    ['channels','報表資料','▦','channels',30,'reports.view'],['alarms','警報中心','△','alarms',40,'alarms.view'],
    ['permissions','帳號權限','♙','permissions',50,'users.manage'],['account','測試','⚗','account',60,'tests.run'],['settings','設定','⚙','settings',70,'settings.manage'],
  ];
  const insertMenu = database.prepare('INSERT OR IGNORE INTO menu_items(menu_key,label,icon,route,sort_order,required_permission) VALUES(?,?,?,?,?,?)');
