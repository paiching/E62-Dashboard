...
function channelReason(channel){const reasons=[];if(channel.status==='read_error')reasons.push('PV／SV 讀取失敗');else if(Number(channel.st)===2)reasons.push('Sensor 異常');else if(channel.pv===null||channel.pv===undefined)reasons.push('PV 無讀值');if(Number(channel.st)===3)reasons.push('讀值連續五筆相同');const pv=Number(channel.pv),low=Number(channel.web_lo),high=Number(channel.web_hi);if(channel.pv!=null&&Number.isFinite(low)&&pv<low)reasons.push(`低於警報下限 ${low} °C`);if(channel.pv!=null&&Number.isFinite(high)&&pv>high)reasons.push(`高於警報上限 ${high} °C`);if(channel.web_alarm)reasons.push('設備警報');return reasons.join('、')||null;}
...
function queryReport(token,filter={}){requireSession(token,'reports.view');const where=reportWhere(filter),limit=Math.min(1000,Math.max(1,Number(filter.limit)||50)),offset=Math.max(0,Number(filter.offset)||0);const total=database.prepare(`SELECT COUNT(*) AS count FROM sensor_readings r JOIN sensor_channels c ON c.channel_id=r.channel_id ${where.sql}`).get(...where.values).count;const rows=database.prepare(`SELECT r.id,r.batch_id,r.channel_id AS id,c.name,r.pv,r.sv,r.state,r.alarm_low AS web_lo,r.alarm_high AS web_hi,r.recorded_at AS time,r.source,r.raw_json FROM sensor_readings r JOIN sensor_channels c ON c.channel_id=r.channel_id ${where.sql} ORDER BY r.recorded_at DESC,CAST(r.channel_id AS INTEGER) LIMIT ? OFFSET ?`).all(...where.values,limit,offset).map(row=>{const raw=parseJson(row.raw_json,{}),reason=channelReason({...raw,pv:row.pv,web_lo:row.web_lo,web_hi:row.web_hi});delete row.raw_json;return{...row,reason};});return{rows,total,limit,offset};}
function csvCell(value){
  const text=value==null?'':String(value);
  // SQLite numeric values are safe; only text needs formula-prefix protection.
  const isNumber=typeof value==='number'&&Number.isFinite(value);
  return `"${(!isNumber&&/^[=+\-@]/.test(text)?`'${text}`:text).replace(/"/g,'""')}"`;
}
async function exportReport(token,filter,browserWindow,dialog){requireSession(token,'reports.export');const where=reportWhere(filter),rows=database.prepare(`SELECT r.channel_id AS id,c.name,r.state,r.pv,r.sv,r.alarm_low AS low,r.alarm_high AS high,r.recorded_at AS time,r.source,r.raw_json FROM sensor_readings r JOIN sensor_channels c ON c.channel_id=r.channel_id ${where.sql} ORDER BY r.recorded_at DESC,CAST(r.channel_id AS INTEGER)`).all(...where.values).map(row=>({...row,reason:channelReason({...parseJson(row.raw_json,{}),pv:row.pv,web_lo:row.low,web_hi:row.high})}));const result=await dialog.showSaveDialog(browserWindow,{title:'匯出 SQLite 報表',defaultPath:`E62報表_${new Date().toISOString().slice(0,10)}.csv`,filters:[{name:'CSV',extensions:['csv']}]});if(result.canceled||!result.filePath)return{canceled:true};const header=['通道','名稱','狀態','異常原因','PV (°C)','SV (°C)','警報下限','警報上限','記錄時間','來源'];const content=`\ufeff${header.map(csvCell).join(',')}\r\n${rows.map((row)=>[row.id,row.name,row.state,row.reason??'',row.pv,row.sv,row.low,row.high,row.time,row.source].map(csvCell).join(',')).join('\r\n')}`;fs.writeFileSync(result.filePath,content,'utf8');return{canceled:false,filePath:result.filePath,rowCount:rows.length};}
