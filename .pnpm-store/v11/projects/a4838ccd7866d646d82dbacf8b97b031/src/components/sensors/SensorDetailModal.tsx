...
import { Activity, BellOff, RotateCcw, Save, SlidersHorizontal, X } from 'lucide-react';
import { StatusBadge } from '../common/StatusBadge';
import { acknowledgeSensorAlarm, clearSensorStats, saveSensorLimits } from '../../services/sensorSettingsService';
...
export function SensorDetailModal({ channel, token, onClose, onChanged }: SensorDetailModalProps) {
  ...
  const stateColor = state === 'alarm' ? '#c93649' : state === 'error' ? '#657384' : '#16856b';
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await onChanged();
      const currentValue = channel.pv;
      const outsideLimits = currentValue !== null && (currentValue < Number(low) || currentValue > Number(high));
      setMessage(`LOW / HIGH 設定已儲存，目前狀態：${outsideLimits ? '警報' : '正常'}`);
  ...
  async function acknowledgeAlarm() {
    setBusy(true); setMessage('');
    try {
      await acknowledgeSensorAlarm(channel.id, token);
      await onChanged();
      setMessage('告警已解除；若目前數值仍超出上下限，系統會再次觸發警報');
    } catch (reason) { setMessage(reason instanceof Error ? reason.message : '解除告警失敗'); }
    finally { setBusy(false); }
  }

  return (
    <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className={`sensor-modal ${state}`} role="dialog" aria-modal="true" aria-labelledby="sensor-modal-title">
        ...
        <div className="sensor-modal-reading" style={{ color: stateColor }}><strong>{value(channel.pv)}</strong><span>°C</span><small>SV {value(channel.sv)} °C</small></div>
        <div className="trend-panel">
          <div><Activity size={16} color={stateColor} /><b>最近趨勢</b><span>{channel.history.length} 筆</span></div>
          <svg viewBox="0 0 100 40" preserveAspectRatio="none" aria-label="最近溫度趨勢圖">
            ...
            {points && <polyline points={points} style={{ stroke: stateColor }} />}
          </svg>
        </div>
        ...
        <form className="sensor-settings-form" onSubmit={submit}>
          ...
          <div className="modal-actions"><button type="button" className="alarm-ack-button" onClick={acknowledgeAlarm} disabled={busy || state !== 'alarm'}><BellOff size={17} />解除告警</button><button type="button" className="secondary-button" onClick={clearStats} disabled={busy}><RotateCcw size={17} />清除 MIN / MAX</button><button className="primary-button" disabled={busy}><Save size={17} />儲存設定</button></div>
        </form>
