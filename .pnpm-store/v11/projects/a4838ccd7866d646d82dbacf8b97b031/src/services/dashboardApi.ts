...
import { applySensorAlarmState } from './sensorSettingsService';
...
export async function fetchDashboardSnapshot(
  ...
  if (runtimeConfig.dataMode === 'mock') {
    ...
    return applySensorAlarmState(createMockSnapshot(200));
  }
  ...
  return applySensorAlarmState(normalizeSnapshot((await response.json()) as DashboardSnapshot));
}
